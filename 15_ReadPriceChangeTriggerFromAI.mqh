...
*/
  ...
  /* handle=FileOpen("AI_T-"+IntegerToString(chart_period)+".csv",FILE_READ|FILE_SHARE_READ);
   if(handle==-1){Comment("Error - file does not exist"); str = "-1"; } 
   if(FileSize(handle)==0){FileClose(handle); Comment("Error - File is empty"); }*/
  
            handle=0; 
       while( handle==0 || handle==-1 ){handle = FileOpen("AI_T-"+IntegerToString(chart_period)+".csv",FILE_CSV|FILE_READ|FILE_SHARE_READ|FILE_WRITE|FILE_SHARE_WRITE);}
