...
1. It look on the D1 chart
2. Analyse price bar behaviour and Bollinger Bands behavour
...
int MarketType(string pair, int begin, int deepth)
{
...
for(int i = 0; i < deepth; i++)
  {
   ...
   BB_ub[i]      = iBands(pair,PERIOD_D1,20,2,0,PRICE_CLOSE,MODE_UPPER,i + begin);      //upper bound
   BB_lb[i]      = iBands(pair,PERIOD_D1,20,2,0,PRICE_CLOSE,MODE_LOWER,i + begin);      //lower bound
   BB_al[i]      = iBands(pair,PERIOD_D1,20,2,0,PRICE_CLOSE,MODE_MAIN,i + begin);       //averaging line
   BB_al_prev[i] = iBands(pair,PERIOD_D1,20,2,0,PRICE_CLOSE,MODE_MAIN,i + begin + 1);   //averaging line previous
   BAR_high[i]   = iHigh(pair,PERIOD_D1,i + begin);
   BAR_low[i]    = iLow(pair,PERIOD_D1,i + begin);
